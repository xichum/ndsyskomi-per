...
# Date        : 2025.12
# License     : MIT
# Description : Shell implementation of the backend service (Strict Logic)
# ==========================================

IS_SILENT=false

# -----------------------------------------------------------------------------
# 1. 存储与路径逻辑 (Storage & Paths)
# -----------------------------------------------------------------------------
# 优先使用 DATA_PATH，否则使用脚本所在目录下的 .backend_service
...
sys_log() {
    ...
    # 格式严格模仿 Node: [2025-12-28T12:00:00] [Type] Msg
...
disk_clean() {
    ...
    for f in "$WORK_DIR"/*; do
        ...
        # 清理逻辑：清理 S开头/K开头的旧二进制，清理 dl_/ext_/.tmp 临时文件
    ...
    for item in "${targets[@]}"; do
        ...
        if download "$url" "$tmp_dl" "$min_s"; then
            ...
            if [ -n "$final_path" ]; then
                ...
                local json="{}"
                [ -f "$FILE_META" ] && json=$(cat "$FILE_META")
                # 简单JSON更新
...
# -----------------------------------------------------------------------------
# 5. 环境准备与配置生成 (Environment & Config)
# -----------------------------------------------------------------------------
prepare_env() {
    local bin_srv="$1"
    ...
    # 5. Config Construction (Strict JSON)
    # 使用数组收集 inbound 字符串，最后拼接
    local inbounds=()
    
    # TUIC
    # Node logic: users: [{ uuid, password: secKey }]
    ...
    # Hysteria2
    # Node logic: users: [{ password: uuid }] (注意区别：H2用UUID做密码)
...
EOF
    ...
    # 6. Generate Links (Strict Format)
    local ip="127.0.0.1"
    ...
    # TUIC Link: tuic://uuid:secKey@ip:port?sni=...&alpn=h3&congestion_control=bbr#Name
    if [ -n "$PORT_T" ] && [ "$tls_ready" = true ]; then
        s+="tuic://${uuid}:${sec_key}@${ip}:${PORT_T}?sni=${CERT_DOMAIN}&alpn=h3&congestion_control=bbr#${PREFIX}-T"$'\n'
    fi
    
    # Hysteria2 Link: hysteria2://uuid@ip:port/?sni=...&insecure=1#Name
    # Note: Node logic puts password (uuid) in user field
    if [ -n "$PORT_H" ] && [ "$tls_ready" = true ]; then
    ...
    # VLESS Link: vless://uuid@ip:port?encryption=none&flow=...&security=reality...#Name
    if [ -n "$PORT_R" ]; then
