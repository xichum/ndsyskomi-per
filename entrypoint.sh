...
IS_SILENT=false
WORK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/.backend_service"

sys_log() {
    local type="$1"
    local msg="$2"
    if [ "$IS_SILENT" = true ] && [ "$type" != "ERR" ]; then return; fi
    echo "[$(date -u +"%T")] [${type}] ${msg}"
}
...
PROBE_URL=$(echo "$PROBE_URL" | xargs)
PROBE_TOK=$(echo "$PROBE_TOK" | xargs)
...
declare -A STATE_PID
declare -A STATE_CRASH_COUNT
declare -A STATE_LAST_START
...
fetch_bin() {
    ...
    local arch=""
    case "$arch_raw" in
        x86_64) arch="amd64" ;; aarch64|arm64) arch="arm64" ;; s390x) arch="s390x" ;;
    esac
    ...
    if [ "$type" == "srv" ]; then
        local tag_data tag
        tag_data=$(curl -s -H "User-Agent: Node" "https://api.github.com/repos/SagerNet/sing-box/releases/latest" || echo "")
        tag=$(echo "$tag_data" | grep -o '"tag_name": *"[^"]*"' | head -1 | cut -d'"' -f4)
        if [ -n "$tag" ]; then
            local ver="${tag#v}" num="${ver//./}"
            targets+=("https://github.com/SagerNet/sing-box/releases/download/${tag}/sing-box-${ver}-linux-${arch}.tar.gz|S${num}_${rand}")
        fi
        targets+=("https://github.com/SagerNet/sing-box/releases/download/v1.12.13/sing-box-1.12.13-linux-${arch}.tar.gz|S11213_${rand}")
    else
        local tag_data tag
        tag_data=$(curl -s -H "User-Agent: Node" "https://api.github.com/repos/komari-monitor/komari-agent/releases/latest" || echo "")
        tag=$(echo "$tag_data" | grep -o '"tag_name": *"[^"]*"' | head -1 | cut -d'"' -f4)
        if [ -n "$tag" ]; then
            local num="${tag#v}"
            num="${num//./}"
            targets+=("https://github.com/komari-monitor/komari-agent/releases/download/${tag}/komari-agent-linux-${arch}|K${num}_${rand}")
        fi
        ...
        if download "$url" "$tmp_dl" "$min_s"; then
            ...
            if [ "$type" == "srv" ]; then
                ...
                if tar -xzf "$tmp_dl" -C "$tmp_ext" 2>/dev/null; then
                    local bin_found
                    bin_found=$(find "$tmp_ext" -type f -name "sing-box" | head -1)
                    [ -n "$bin_found" ] && final_path="$WORK_DIR/$name" && mv "$bin_found" "$final_path"
                fi
                rm -rf "$tmp_ext"
            else
            ...
            if [ -n "$final_path" ] && [ -f "$final_path" ]; then
                ...
                local fname
                fname=$(basename "$final_path")
                local content="{}"
                [ -f "$FILE_META" ] && content=$(cat "$FILE_META")
...
prepare_env() {
    ...
    local uuid="$UUID_ENV"
    if [ -z "$uuid" ]; then
        if [ -f "$FILE_TOKEN" ]; then uuid=$(cat "$FILE_TOKEN" | xargs)
        else
    ...
    if { [ -n "$PORT_T" ] || [ -n "$PORT_H" ]; }; then
        if [ -n "$CERT_URL" ] && [ -n "$KEY_URL" ]; then sys_log "Init" "Syncing assets..."; download "$CERT_URL" "$FILE_CERT"; download "$KEY_URL" "$FILE_KEY"; fi
        if ! check_tls; then
             local out
             out=$("$bin_srv" generate tls-keypair "$CERT_DOMAIN" 2>/dev/null)
             local k c
             k=$(echo "$out" | sed -n '/BEGIN PRIVATE KEY/,/END PRIVATE KEY/p')
             c=$(echo "$out" | sed -n '/BEGIN CERTIFICATE/,/END CERTIFICATE/p')
             [ -n "$k" ] && [ -n "$c" ] && { save_file "$FILE_KEY" "$k" 600; save_file "$FILE_CERT" "$c"; }
        fi
    fi
    local tls_ready=false
    check_tls && tls_ready=true
    ...
    if [ -n "$PORT_H" ] && [ "$tls_ready" = true ]; then
        local obfs_part=""
        [ "$HY2_OBFS" == "true" ] && obfs_part="\"obfs\": { \"type\": \"salamander\", \"password\": \"$sec_key\" },"
    ...
    local ip="127.0.0.1" ext_ip
    ext_ip=$(curl -s --connect-timeout 3 https://api.ipify.org)
...
filter_log() {
    ...
    while IFS= read -r line; do
        ...
        if echo "$msg" | grep -qE "error|fatal|panic"; then sys_log "ERR" "[$prefix] Runtime exception"; else sys_log "$prefix" "${msg:0:50}"; fi
    done
}
    ...
    while true; do
        ...
        for key in "srv" "mon"; do
            ...
            if ! kill -0 "$pid" 2>/dev/null; then
                 ...
                 local count="${STATE_CRASH_COUNT[$key]}" delay=0
                 if [ "$count" -gt 0 ]; then
                    delay=$((2000 * (2 ** count)))
                    [ "$delay" -gt 60000 ] && delay=60000
                 fi
                 local label="Core"
                 [ "$key" == "mon" ] && label="Agent"
                 sys_log "Sys" "$label reload in $((delay/1000))s"
                 sleep $((delay/1000))
                 [ "$key" == "srv" ] && spawn_service "srv" "$bin_srv" "run" "-c" "$FILE_CONF" || spawn_service "mon" "$bin_mon" "-e" "$u" "-t" "$PROBE_TOK"
            fi
        done
        check_cron
        sleep 2
...
check_cron() {
    ...
    local now_sec
    now_sec=$(date +%s)
    if [ "$NEXT_CRON_TS" -eq 0 ] || [ "$now_sec" -ge "$NEXT_CRON_TS" ]; then
        local cron_str="$CRON"
        ...
        if [[ "$cron_str" =~ UTC([+-]?[0-9]+)[[:space:]]+([0-9]+):([0-9]+) ]]; then
            ...
            local utc_now
            utc_now=$(date -u +%s)
            local offset_sec=$((off * 3600)) target_zone_now=$((utc_now + offset_sec))
            local target_day
            target_day=$(date -u -d "@$target_zone_now" +%Y-%m-%d)
            local target_ts
            target_ts=$(date -u -d "$target_day $h:$min:00" +%s)
...
start_http_server() {
    ...
    local nc_flags="-l -p $PORT_WEB"
    $nc_cmd -h 2>&1 | grep -q "\-q" && nc_flags="$nc_flags -q 1"
    ...
    while true; do
        { echo -ne "HTTP/1.1 200 OK\r\n";
          ...
          if [[ "$path" == "/api/data"* ]] && [ -f "$FILE_SUB" ]; then
               ...
               if [ "$ok" = true ]; then local tick=$(( ( $(date +%s%3N) - STATE_LAST_START["srv"] ) / 1000 )); echo -ne "Content-Type: application/json\r\n\r\n"; echo -n "{\"status\": \"OK\", \"tick\": $tick}"; else echo -ne "Content-Type: application/json\r\n\r\n"; echo -n "{\"status\": \"ERR\", \"tick\": 0}"; fi
          else
...
if [ -z "$BIN_SRV" ]; then echo "Fatal: Could not fetch binary"; exit 1; fi
