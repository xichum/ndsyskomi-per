...
const CONFIG = {
  ...
  WORK_DIR: path.join(__dirname, '.npm'),
...
class ResourceTuner {
  ...
  detect() {
    ...
    sysLog('sys', `RAM:${Math.round(this.memBytes/1024/1024)}MB CPU:${this.cpuCores}`);
  }
  getEnv() {
...
// ==============================================================================
// [4] INSTALLATION (Fixed Permissions)
// ==============================================================================

async function installCore() {
  let ver = "1.10.7";
  try {
    const res = await axios.get('https://github.com/SagerNet/sing-box/releases/latest', { maxRedirects: 0, validateStatus: null, timeout: 5000 });
    if (res.headers.location) ver = res.headers.location.match(/v(\d+\.\d+\.\d+)/)[1];
  } catch (e) {}
  ...
  if (meta.version !== ver || !binPath || !fs.existsSync(binPath)) {
    ...
    // [CRITICAL FIX] Explicitly set executable permissions
...
async function genLinks(uuid, pub) {
  ...
  if (txt) {
    ...
    global.SUB_PATH = f;
    console.log(`\n\x1b[32m[SUB]\x1b[0m ${b64.substring(0, 50)}...\n${b64}\n`);
  }
}
